...
class AGE_32(object):
	...
	def sample(self, x, sample_size, sample_seed):
		...
		d = {self.x_placeholder: x[:int(sample_size*sample_size/2), :, :, :], self.z_placeholder: z,
	...
	def encoder(self, image, reuse = False, scope_str = "Encoder"):
		with tf.variable_scope(scope_str) as scope:
			...
			conv2 = tf.reshape(conv2, shape=[-1, 8 * 8 * 2 * self.df_dim])
...
def sampleModel(opt):
	...
	if(opt.output_dir == 'None'):
		opt.output_dir = "../output/" + opt.dataset + "32"
